...
const DosenSidebar = ({ isCollapsed, onToggle }) => {
  ...
  const menuItems = [
    ...
    },
    {
      title: 'Mata Kuliah',
      icon: BookOpen,
      path: '/dosen/dashboard/courses'
    },
    {
      title: 'Mahasiswa',
      icon: Users,
      path: '/dosen/dashboard/students'
    },
    {
      title: 'Tugas & Proyek',
      icon: FileText,
      path: '/dosen/dashboard/assignments'
    },
    {
      title: 'Penilaian',
      icon: BarChart3,
      path: '/dosen/dashboard/grading'
    },
    {
      title: 'Jadwal',
      icon: Calendar,
      path: '/dosen/dashboard/schedule'
    }
  ];
    ...
    <div className={`bg-white shadow-lg transition-all duration-300 ${
      ...
      <div className="p-4 border-b border-gray-200 flex-shrink-0">
        <div className="flex items-center justify-between">
          {!isCollapsed && (
            <div>
              <h2 className="text-xl font-bold text-gray-800">Portal Dosen</h2>
      ...
      <div className="p-4 border-t border-gray-200 flex-shrink-0">
        <ul className="space-y-2">
          <li>
            <NavLink
              to="/dosen/dashboard/settings"
              className={`flex items-center p-3 rounded-lg transition-all duration-200 ${
                isActive('/dosen/dashboard/settings')
                  ? 'bg-blue-50 text-blue-700'
                  : 'text-gray-700 hover:bg-gray-50 hover:text-blue-600'
              }`}
              title={isCollapsed ? 'Pengaturan' : ''}
            >
              <Settings className={`w-5 h-5 ${isCollapsed ? 'mx-auto' : 'mr-3'}`} />
